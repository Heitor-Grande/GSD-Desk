...
export function CampoTexto({
    ...
    return (
        <div className={`min-w-0 max-w-full ${className}`}>
            ...
            {labelVisivel && (
                <label htmlFor={id} className="block break-words text-sm font-semibold text-slate-700">
                    {label}
                </label>
            )}
            ...
            {
                helpText && (
                    <small className={`${classNameHelpText} break-words`}>
                        {helpText}
                    </small>
                )
            }
