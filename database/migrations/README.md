...
Criada por `009_criar_tabela_ticket.sql`.
...
- `status`: status atual do ticket com até 30 caracteres.
...
A tabela `auditoria` armazena registros de altera��es relevantes realizadas na aplica��o.

Campos:

- `id`: chave prim�ria.
- `acao`: a��o realizada, como `CREATE`, `UPDATE` ou `DELETE`.
- `usuario_id`: usu�rio que realizou a a��o.
- `empresa_id`: empresa relacionada ao contexto da a��o, quando houver.
- `dados_antes`: dados antes da altera��o, usado em `UPDATE` e `DELETE`.
- `dados_depois`: dados depois da altera��o, usado em `UPDATE`.
- `criado_em`: data de cria��o do registro de auditoria.

�ndices e relacionamentos:

- `auditoria_pkey`: chave prim�ria em `id`.
...
- `auditoria_acao_idx`: �ndice para consultas por a��o.
- `auditoria_usuario_id_idx`: �ndice para consultas por usu�rio.
- `auditoria_empresa_id_idx`: �ndice para consultas por empresa.
- `auditoria_criado_em_idx`: �ndice para consultas por data de cria��o.

Regras importantes:

- Em a��es de `CREATE`, `dados_antes` e `dados_depois` n�o devem ser preenchidos.
- Em a��es de `UPDATE`, preencha `dados_antes` com o estado anterior e `dados_depois` com o estado posterior.
- Em a��es de `DELETE`, preencha `dados_antes` com o estado anterior e n�o preencha `dados_depois`.
